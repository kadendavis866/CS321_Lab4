...
public class RandomAccessExample 
{
		...
		throws IOException
	{
		...
		System.out.println("file size before  appending = "
		                          +datafile.length()+" bytes");
		...
		System.out.println("file size after appending  = "
		                         +datafile.length()+" bytes");
		...
		data.writeInt(128);
		System.out.println("after random access write = "
		                            +datafile.length()+" bytes");
		data.seek(4);
		// the integer at that offset should be 1
		System.out.println("integer read from offset 4 was:"+data.readInt());
